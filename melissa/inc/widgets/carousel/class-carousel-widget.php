 ...
 * @package Melissa
 */
if ( ! class_exists( 'Melissa_Carousel_Widget' ) ) {

	class Melissa_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description	= esc_html__( 'Display a list of your posts on your site.', 'melissa' );
			$this->widget_id			= 'melissa_widget_carousel';
			$this->widget_name			= esc_html__( 'Carousel', 'melissa' );
			$this->utility				= melissa_utility()->utility;
			$this->settings				= array(
				'title' => array(
					'type'				=> 'text',
					'value'				=> '',
				...
				'terms_type' => array(
					...
					'options' => array(
						'category' => array(
							'label'		=> esc_html__( 'Category', 'melissa' ),
							'slave'		=> 'terms_type_category',
						),
						'post_tag' => array(
							'label'		=> esc_html__( 'Tag', 'melissa' ),
							'slave'		=> 'terms_type_post_tag',
						),
					),
					'label' => esc_html__( 'Choose taxonomy type', 'melissa' ),
				),
				'categories' => array(
					'type'				=> 'select',
					'size'				=> 1,
					...
					'options'			=> false,
					'label'				=> esc_html__( 'Select category', 'melissa' ),
					'multiple'			=> true,
					'placeholder'		=> esc_html__( 'Select category', 'melissa' ),
				...
				'tags' => array(
					...
					'options'			=> false,
					'label'				=> esc_html__( 'Select tags', 'melissa' ),
					'multiple'			=> true,
					'placeholder'		=> esc_html__( 'Select tags', 'melissa' ),
				...
				'posts_per_page' => array(
					...
					'max_value'			=> 20,
					'min_value'			=> 1,
					'label'				=> esc_html__( 'Posts count', 'melissa' ),
				),
				'post_title' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display title', 'melissa' ),
				),
				'content' => array(
					'type'				=> 'switcher',
					'value'				=> 'false',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display content', 'melissa' ),
				),
				'more_button' => array(
					'type'				=> 'switcher',
					'value'				=> 'false',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display more button', 'melissa' ),
					'toggle'			=> array(
						'true_toggle'	=> esc_html__( 'On', 'melissa' ),
						'false_toggle'	=> esc_html__( 'Off', 'melissa' ),
				...
				'more_button_text' => array(
					'type'				=> 'text',
					'value'				=> esc_html__( 'Read more', 'melissa' ),
					'label'				=> esc_html__( 'More button text', 'melissa' ),
				...
				'trim_words' => array(
					...
					'min_value'			=> 1,
					'step_value'		=> 1,
					'label'				=> esc_html__( 'Content words trimmed count', 'melissa' ),
				),
				'slides_per_view' => array(
					...
					'min_value'			=> 1,
					'value'				=> 5,
					'label'				=> esc_html__( 'Number of slides per view', 'melissa' ),
				),
				'slides_per_group' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Number slides per group', 'melissa' ),
				),
				'slides_per_column' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Multi Row Slides Layout', 'melissa' ),
				),
				'space_between_slides' => array(
					...
					'min_value'			=> 0,
					'value'				=> 30,
					'label'				=> esc_html__( 'Width of the space between slides(px)', 'melissa' ),
				),
				'duration_speed' => array(
					...
					'min_value'			=> 100,
					'value'				=> 500,
					'label'				=> esc_html__( 'Duration of transition between slides (ms)', 'melissa' ),
				),
				'navigation' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider navigation', 'melissa' ),
				),
				'pagination' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider pagination', 'melissa' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', melissa_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="melissa-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'melissa' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'melissa_carousel_default_query_args', array(
				'post_type'			=> 'post',
				'orderby'			=> 'date',
				'order'				=> 'DESC',
				'posts_per_page'	=> -1,
		...
		public function get_carousel_loop( $posts_query ) {
			...
			if ( ! $view_dir ) {
				return '<h5>' . esc_html__( 'View file not found', 'melissa' ) . '</h5>';
			}
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				...
				$image = $this->utility->media->get_image( array(
					...
					'mobile_size'		=> 'melissa-thumb-1170-600',
	...
	add_action( 'widgets_init', 'melissa_register_carosel_widgets' );
	function melissa_register_carosel_widgets() {
		register_widget( 'Melissa_Carousel_Widget' );
	}
}
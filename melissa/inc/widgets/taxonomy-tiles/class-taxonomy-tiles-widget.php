...
if ( ! class_exists( 'Melissa_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Melissa_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Taxonomy Tiles', 'melissa' );
			$this->widget_description	= esc_html__( 'This widget displays images from taxonomy.', 'melissa' );
			$this->widget_id			= 'melissa-widget-taxonomy-tiles';
			...
			$this->settings				= array(
				'title'	=> array(
					...
					'label'				=> esc_html__( 'Widget title', 'melissa' ),
				),
				'terms_type' => array(
					'type'				=> 'radio',
					'value'				=> 'category',
				...
				'category'=> array(
					...
					'label'				=> esc_html__( 'Select category to show', 'melissa' ),
				...
				'post_tag' => array(
					...
					'label'				=> esc_html__( 'Select tags to show', 'melissa' ),
				...
				'description_length' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Description words length ( Set 0 to hide description. )', 'melissa' ),
				),
				'show_post_count' => array(
					...
					'options'		=> array(
						'show_post_count_check' => esc_html__( 'Show post count', 'melissa' ),
					),
				),
				'layout_type' => array(
					'type'				=> 'radio',
					...
					'options'			=> array(
						'grid' => array(
							'label'		=> esc_html__( 'Grid', 'melissa' ),
							'slave'		=> 'layout_type_grid',
						),
						'tiles' => array(
							'label'		=> esc_html__( 'Tiles', 'melissa' ),
						),
					),
					'label'				=> esc_html__( 'Choose Layout Type', 'melissa' ),
				),
				'columns_number' => array(
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Columns number ( layout type grid )', 'melissa' ),
				...
				'items_padding' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Items padding ( size in pixels )', 'melissa' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				echo apply_filters( 'melissa_taxonomy_tiles_widget_before', '<div class="row grid ' . $layout_type . '-columns columns-number-' . $columns_number . '" style="margin: 0 0 0 -' . $items_padding . 'px" >', $instance );
				...
				foreach ( $terms as $term_key => $term ) {
					...
					if ( $view_dir ){
						$title = $this->utility->attributes->get_title(
							array(
								'html'			=> '<h3 %1$s><a href="%2$s" %3$s>%4$s</a></h3>',
						...
						$count = $this->utility->meta_data->get_post_count_in_term(
							array(
								...
								'sufix'			=> _n_noop( '%s post', '%s posts', 'melissa' ),
				...
				echo apply_filters( 'melissa_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'melissa_register_taxonomy_tiles_widget' );
	function melissa_register_taxonomy_tiles_widget() {
		register_widget( 'Melissa_Taxonomy_Tiles_Widget' );
	}

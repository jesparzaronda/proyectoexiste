...
class Melissa_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'melissa' );
		$this->widget_id          = 'melissa_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'melissa' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'melissa' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'melissa' ),
				'label' => esc_html__( 'Subscribe Title', 'melissa' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'melissa' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'melissa' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'melissa' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'melissa' ),
				'label' => esc_html__( 'Subscribe submit label', 'melissa' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'melissa' ),
				'label' => esc_html__( 'Subscribe success', 'melissa' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'melissa' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'melissa' ),
				'label' => esc_html__( 'Follow Title', 'melissa' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'melissa' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'melissa' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'melissa' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'melissa' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'melissa' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'melissa' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'melissa' ),
					'top-center'    => esc_html__( 'Top Center', 'melissa' ),
					'top-right'     => esc_html__( 'Top Right', 'melissa' ),
					'center-left'   => esc_html__( 'Middle Left', 'melissa' ),
					'center'        => esc_html__( 'Middle Center', 'melissa' ),
					'center-right'  => esc_html__( 'Middle Right', 'melissa' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'melissa' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'melissa' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'melissa' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'melissa' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'melissa' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'melissa' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'melissa' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'melissa' ),
					'contain' => esc_html__( 'Contain', 'melissa' ),
					'auto'    => esc_html__( 'Auto', 'melissa' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'melissa' ),
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_melissa_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_melissa_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'melissa' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'melissa' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'melissa' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'melissa' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'melissa' );
		}
	...
	public function get_social_nav() {
		return melissa_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = melissa_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'melissa_subscribe' ) ) {
...
add_action( 'widgets_init', 'melissa_register_subscribe_follow_widgets' );
function melissa_register_subscribe_follow_widgets() {
	register_widget( 'melissa_Subscribe_Follow_Widget' );
}

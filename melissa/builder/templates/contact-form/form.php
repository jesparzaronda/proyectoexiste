...
if ( $title !== "" ){
?>
<h2 class="tm_pb_contact_main_title">
...
<div class="tm-pb-contact-message">
	<?php echo $this->_var( 'tm_error_message' ); ?>
</div>

<form class="tm_pb_contact_form clearfix" method="post"
      action="<?php echo esc_url( get_permalink( get_the_ID() ) ); ?>">
	<div class="row">
		<?php echo $this->_var( 'content' ); ?>
	</div>
	<input type="hidden" value="tm_contact_proccess"
	       name="<?php echo $this->cf_id( 'tm_pb_contactform_submit_%s' ); ?>">
	<input type="text" value="" name="<?php echo $this->cf_id( 'tm_pb_contactform_validate_%s' ); ?>"
